...
##### `wethepeople_issue_class_prefix`

Enables site owners to change the prefix on issue name/id CSS classes generated by `We_The_People_Entity::get_classes()`. Defaults to `issue-`.

*Added in version 1.1*

###### Arguments

* `$prefix` (str) The prefix to apply to issue-based CSS class names (default: `issue-`)

###### Return value

String (the prefix to apply)

###### Example

```php
/**
 * Change the 'issue-' prefix on CSS classes to 'topic-'
 * e.g. 'issue-gun-control' becomes 'topic-gun-control'
 * @return str
 */
function set_issue_css_class_prefix() {
  return 'topic-';
}
add_filter( 'wethepeople_issue_class_prefix', 'set_issue_css_class_prefix' );
```
...
##### `wethepeople_status_class_prefix`

Enables site owners to change the prefix on status-based CSS classes generated by `We_The_People_Entity::get_classes()`. Defaults to `status-`.

*Added in version 1.1*

###### Arguments

* `$prefix` (str) The prefix to apply to status-based CSS class names (default: `status-`)
...
```php
/**
 * Change the 'status-' prefix on CSS classes to 'petition-is-'
 * e.g. 'status-open' becomes 'petition-is-open'
 * @return str
 */
function set_status_css_class_prefix() {
  return 'petition-is-';
}
add_filter( 'wethepeople_status_class_prefix', 'set_status_css_class_prefix' );
```
